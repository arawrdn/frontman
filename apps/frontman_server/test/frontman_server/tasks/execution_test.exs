defmodule FrontmanServer.Tasks.ExecutionIntegrationTest do
  ...
  # -- Interactive tool (question) suspend / resume --------------------------

  describe "interactive tool (question) suspend and resume" do
    setup :setup_task

    test "question tool suspends execution and broadcasts :agent_suspended", %{
      task_id: task_id,
      scope: scope
    } do
      question_tc = tool_call("question", question_args())
      ...
      assert_receive :agent_suspended, 5_000
      ...
      tool_calls =
        Enum.filter(task.interactions, &match?(%Interaction.ToolCall{tool_name: "question"}, &1))
    ...
    test "submitting tool result after suspension resumes execution", %{
      task_id: task_id,
      scope: scope
    } do
      question_tc_id = "tc_question_resume_#{System.unique_integer([:positive])}"
      ...
      Tasks.maybe_resume_after_tool_result(scope, task_id, [],
        agent: agent,
        mcp_tool_defs: question_mcp_tool_defs()
      )
