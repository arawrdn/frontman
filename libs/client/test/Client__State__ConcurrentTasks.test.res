...
describe("Concurrent Tasks Event Routing", () => {
  ...
  test("TextDeltaReceived event routes to correct task", t => {
    ...
    let (finalState, _) = StateReducer.next(
      stateWithMessage,
      TaskAction({target: ForTask(taskAId), action: TextDeltaReceived({text: "Hello from Task A", timestamp: None})}),
    )
  ...
  test("Multiple concurrent tasks streaming simultaneously", t => {
    ...
    let (state4, _) = StateReducer.next(state3, TaskAction({target: ForTask(taskAId), action: TextDeltaReceived({text: "A", timestamp: None})}))
    let (state5, _) = StateReducer.next(state4, TaskAction({target: ForTask(taskBId), action: TextDeltaReceived({text: "B", timestamp: None})}))
    let (finalState, _) = StateReducer.next(state5, TaskAction({target: ForTask(taskCId), action: TextDeltaReceived({text: "C", timestamp: None})}))
  ...
  test("TurnCompleted event routes to correct task", t => {
    ...
    let (stateWithText, _) = StateReducer.next(
      stateWithStream,
      TaskAction({target: ForTask(taskAId), action: TextDeltaReceived({text: "Complete message", timestamp: None})}),
    )
  ...
  test("Switching current task mid-stream doesn't affect event routing", t => {
    ...
    let (stateWithText1, _) = StateReducer.next(
      stateWithStream,
      TaskAction({target: ForTask(taskAId), action: TextDeltaReceived({text: "Part 1. ", timestamp: None})}),
    )
    ...
    let (finalState, _) = StateReducer.next(
      stateWithB,
      TaskAction({target: ForTask(taskAId), action: TextDeltaReceived({text: "Part 2.", timestamp: None})}),
    )
