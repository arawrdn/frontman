...
let make = (~apiBaseUrl: string) => {
  ...
  let {connectionState, sendPrompt, cancelPrompt, respondToElicitation, loadTask, deleteSession, authRedirectUrl, _} = Client__FrontmanProvider.useFrontman()
  ...
  React.useEffect(() => {
    ...
    | Connected | SessionActive(_) =>
      Client__Debug.init()
      Client__State.Actions.setAcpSession(~sendPrompt, ~cancelPrompt, ~loadTask, ~deleteSession, ~respondToElicitation, ~apiBaseUrl)
    | Disconnected | Error(_) => Client__State.Actions.clearAcpSession()
    | _ => ()
    }
    None
  }, (connectionState, sendPrompt, cancelPrompt, respondToElicitation, loadTask, deleteSession, apiBaseUrl))
