    ...
    | AcpSessionActive({loadTask}) =>
      ...
      loadTask(taskId, ~needsHistory, ~metadata, ~onComplete=result => {
        ...
        | Ok() =>
          ...
          // During history replay, agent_message_chunk events go through the
          // rAF-based textDeltaBuffer. Without this flush, the last agent
          // message chunk can arrive after LoadComplete transitions the task
          // to Loaded({isAgentRunning: false}), where TextDeltaReceived is
          // silently dropped — causing the final LLM reply to disappear.
          Client__TextDeltaBuffer.flush()
