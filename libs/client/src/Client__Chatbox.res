...
let make = () => {
  ...
  <div
    ...
    <div
      ...
      <AIElements.Conversation className="h-full">
          ...
          ->Array.mapWithIndex((message, i) => {
            ...
            <div key={messageId}>
              {message.role === "assistant" && sourceUrls->Array.length > 0
                ? <AIElements.Sources>
                    ...
                    ->Array.mapWithIndex((part, j) => {
                      part.url->Option.mapOr(React.null, url =>
                        <AIElements.SourcesContent key={`${messageId}-${j->Int.toString}`}>
                          <AIElements.Source
                            key={`${messageId}-${j->Int.toString}`}
                            href={url}
                            title={url}
                          />
              ...
              ->Array.mapWithIndex((part, j) => {
                ...
                if part.type_ === "text" {
                  part.text->Option.mapOr(React.null, text =>
                    <React.Fragment key={partKey}>
                      ...
                      {message.role === "assistant" && i === messages->Array.length - 1
                        ? <AIElements.Actions className="mt-2">
                            ...
                            <AIElements.Action
                              onClick={() => {
                                let _ = WebAPI.Global.navigator.clipboard
                                ->WebAPI.Clipboard.writeText(text)
                              }}
                              label="Copy">
                  ...
                  part.text->Option.mapOr(React.null, text =>
                    <AIElements.Reasoning
                      ...
                      message.id === messages->Array.get(messages->Array.length - 1)->Option.mapOr("", m => m.id)}>
        ...
        <AIElements.PromptInputFooter>
          <AIElements.PromptInputTools>
            ...
            <AIElements.PromptInputModelSelect
              onValueChange={value => setModel(_ => value)}
              value={model}>
          ...
          <AIElements.PromptInputSubmit disabled={input === "" && status !== "streaming"} status={status} />
        </AIElements.PromptInputFooter>
      </AIElements.PromptInput>
    </div>
  </div>
