...
module Node = EventBus__Bindings__Node

// Helper to spawn a compiled ReScript fixture
let spawnFixture = (fixtureName: string): EventBus__Bindings__ChildProcess.childProcess => {
  let fixturePath = Node.Path.join([Node.__dirname, "../test", fixtureName ++ ".res.mjs"])
...
let wait = async (ms: int): unit => {
  await Js.Promise2.make((~resolve, ~reject as _) => {
    let _ = Node.setTimeout(() => resolve(), ms)
  })
}
